...
export class Bot {
  ...
  help(channel: TextBasedChannels, member: GuildMember): void {
    const printCommand = (command: Command): string => {
      ...
      const aliases = command.aliases
        ? `, aliases: \`${this.prefix}${command.aliases.join(
            `\`, \`${this.prefix}`
          )}\``
        : "";
    ...
    for (const [name, command] of this.commands.entries()) {
      const [hasPermission, permissionName] = this._checkPermissions(
        command,
        member
      );
  ...
  _checkRoles(
    ...
    for (const roleName of command.roles) {
      const cmdRole = member.guild.roles.cache.find(
  ...
  _canUseCommand(
    ...
    if (!hasPermission) {
      return [
        false,
        `You don't have the permission ${permissionName} to use this command.`,
      ];
    }
  ...
  _listen(): void {
    this.client.on("messageCreate", (message) => {
      ...
      const [canUseCommand, errorMessage] = this._canUseCommand(
        command,
        member
      );
      if (!canUseCommand) {
      ...
      if (
        ...
        channel.send(
          `Incorrect usage! Use \`${this.prefix}${`${command.name} ${
            expectedArgs || ""
          }`.trimEnd()}\``
        );
        return;
    ...
    this.client.on("interactionCreate", async (interaction) => {
      ...
      if (!command) {
        interaction.reply(
          `Slashcommand \`${interaction.commandName}\` not found.`
        );
        return;
      }
      ...
      if (
        ...
        interaction.reply(
          `Slashcommand \`${interaction.commandName}\` found but not configured.`
        );
        return;
      }
